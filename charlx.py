...
class AtomsTSOptimizer:
    """
    AtomsTSOptimizer class for transition state optimization using Sella.
    Applies only fixed constraints during TS optimization.
    ...
    def optimize(self, solutions, gen=None):
        ...
        optimized_atoms_list = calc_atoms_list(
            atoms_list=atoms_list,
            func=self.optimize_atoms,
            desc="TS Optimization",
            multiproc=self.multiproc,
            n_proc=self.n_proc,
            progress_bar=self.progress_bar,
            kwargs={"steps": self.steps, "gen": gen},
    ...
    def optimize_atoms(
        ...
        if self.save_traj and self.traj_path is not None and gen is not None:
            os.makedirs(self.traj_path, exist_ok=True)
            traj_filename = os.path.join(self.traj_path, f"gen_{gen}_ts.traj")
            dyn = Sella(
                atoms=atoms,
                logfile=self.logfile,
                trajectory=traj_filename,
            ...
            dyn = Sella(atoms=atoms, logfile=self.logfile)
    ...
    def __str__(self):
        return f"AtomsTSOptimizer(founder_atoms={self.founder_atoms}, fixed_indices={self.fixed_indices}, frozen_indices={self.frozen_indices}, free_indices={self.free_indices}, calc={self.calc}, fmax={self.fmax:.5f}, steps={self.steps}, logfile={self.logfile}, multiproc={self.multiproc}, n_proc={self.n_proc}, device={self.device}, save_traj={self.save_traj}, traj_path={self.traj_path}, save_interval={self.save_interval})"
